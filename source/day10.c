...
when you open the book, you discover that most of it seems to have been scorched
by lava! As you're about to ask how you can help, the reindeer brings you a blank
topographic map of the surrounding area (your puzzle input) and looks up
t you excitedly.
...
trail is as long as possible and has an even, gradual, uphill slope.
...
height of exactly 1 at each step. Hiking trails never include diagonal
...
to construct a small pile of pencils, markers, rulers, compasses, stickers, and other equipment you might need to update the map with hiking trails.
...
they have scores of 5, 6, 5, 3, 1, 3, 5, 3, and 5. Adding these scores together,
...
#define INPUTS_PATH "../inputs/day10.txt"

uint32_t count_paths(size_t rows_n,
                     size_t cols_n,
...
int main()
{
    ...
    char(*char_board)[cols_n] = malloc(sizeof(*char_board) * rows_n);

    char *line = inputs;
    for (int i = 0; i < rows_n; i++)
    {
        memcpy(char_board[i], line, cols_n);
        line = strstr(line, "\n") + 1;
    }
    free(inputs);
    inputs = NULL;

    int(*board)[cols_n] = malloc(sizeof(*board) * rows_n);
    for (int i = 0; i < rows_n; i++)
    {
        for (int j = 0; j < cols_n; j++)
        {
            board[i][j] = char_board[i][j] - '0';
        }
    }
    free(char_board);
    char_board = NULL;
